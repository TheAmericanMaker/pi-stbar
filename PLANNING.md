...
- Desktop CORS fix (IMPLEMENTED, verification pending James's rebuild) — provider/Ollama requests routed through `tauri-plugin-http` (native Rust HTTP client) via a Tauri-only, cross-origin-only `window.fetch` shim (`web/src/tauri-tools.ts` `installTauriFetch`); intended to let local Ollama connect in the packaged app with no `OLLAMA_ORIGINS`/proxy/flags. Reason: packaged webview origin `http://tauri.localhost` is rejected by Ollama's CORS — server-side requests have no origin. Needs `npm install` (web) + `tauri` rebuild to confirm — 2026-05-20
