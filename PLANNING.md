...
- Desktop CORS fix — provider/Ollama requests routed through `tauri-plugin-http` (native Rust HTTP client) via a Tauri-only, cross-origin-only `window.fetch` shim (`web/src/tauri-tools.ts` `installTauriFetch`); local Ollama now connects in the packaged app with no `OLLAMA_ORIGINS`/proxy/flags. Reason: packaged webview origin `http://tauri.localhost` is rejected by Ollama's CORS — server-side requests have no origin — 2026-05-20
